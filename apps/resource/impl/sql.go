...
const (
	...
	WHERE id = ?`
	slqDeleteResource = `DELETE FROM resource WHERE id =?`
	...
	//-- 用于分页时使用,获取总数
	sqlCountResource = `
	SELECT COUNT(DISTINCT r.id) FROM resource r %s JOIN resource_tag t ON r.id =t.resource_id`

	sqlQueryResourceTag = `
		SELECT t_key,t_value,description,resource_id,weight,type FROM resource_tag`

	slqDeleteResourceTag = `
		DELETE FROM resource_tag WHERE resource_id =? AND t_key =? AND t_value =?;`
